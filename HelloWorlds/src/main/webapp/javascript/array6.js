...
function findVal() {
    ...
    const findSalary = document.querySelectorAll('input')[1].value;
    console.log(findSalary);

    let chk = data.some(function (val, idx) {
        return val.email.indexOf(findEmail) == -1;
    });
    ...
    //     return (val.gender === findGen && (val.email).indexOf(findEmail) != -1 && val.salary > findSalary);
    // });

    // console.log(filterData);

    let filterData = data.reduce(function (acc, obj, ind) {
        if(obj.gender === findGen && obj.email.indexOf(findEmail) != -1 && obj.salary > findSalary) {
            console.log(obj);
            acc.push(obj);
        }
        return acc;
